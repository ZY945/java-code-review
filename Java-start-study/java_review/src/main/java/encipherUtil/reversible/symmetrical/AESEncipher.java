package encipherUtil.reversible.symmetrical;
...
public class AESEncipher {
    public static void main(String[] args) throws UnsupportedEncodingException {
        Key key = AESEncipher.generateKeyPair();
        byte[] encoded = key.getEncoded();
        String s2 = Base64Util.encoderGetStrByByte(encoded);
        System.out.println("密钥BASE64" + s2);
        System.out.println(ByteConverter.byteToHexString(encoded));
        String data = "test中文";
        String s = AESEncipher.encryptedByECB(data, key);
        System.out.println("文本BASE64" + s);
        byte[] bytes = Base64Util.decoderGetByteByStr(s);
        System.out.println(ByteConverter.byteToHexString(bytes));
        String s1 = AESEncipher.decryptedByECB(s, key);
